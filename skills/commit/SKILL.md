...
모노레포 환경에서는 다음 규칙을 추가로 적용합니다.

### Scope 추론

- `apps/<name>/...` → scope는 `<name>`
- `packages/<name>/...` → scope는 `<name>`
- 루트 설정 파일 변경 (turbo.json, pnpm-workspace.yaml 등) → scope 생략 또는 `root`

### 커밋 분리

- **패키지가 다르면 커밋을 분리한다.** 같은 type이라도 `packages/core`와 `apps/web` 변경은 별도 커밋.
- **패키지 간 의존성이 있으면 의존되는 쪽을 먼저 커밋한다.** (예: core 변경 → web에서 core 사용 → core 먼저 커밋)
- **루트 설정 변경은 별도 커밋으로 분리한다.**

예시:
```
1. feat(core): 새 유틸 함수 추가
2. feat(web): core 유틸 함수 활용한 기능 구현
3. chore: turbo.json 빌드 파이프라인 수정
```
